...
class MultiGrid(object):
    ...
    def get_grid(self, symbol="", gid=-1):
        ...
        Notes:
                One of the two arguments have to be provided, ``symbol``
                has priority over ``gid`` if both are provided.
        """
    ...
    def get_common_points(self, symbol_a, symbol_b):
        ...
        Returns:
                output (ndarray): Array with the index in which the cartesian
                points are the same. The size of the array is the number of
                common points ``n`` with the shape ``(n, 2)`` in which
                ``[:,0]`` corresponds to de index for ``a`` while ``[:,1]``
                are the index for ``b``.
        """
        ...
        if idx_a is not None and idx_b is not None:
            ptr = napmo.cext.MultiGrid_get_common_index(
                self._this, idx_a, idx_b)
            size = napmo.cext.MultiGrid_get_common_index_size(
                self._this, idx_a, idx_b)
