...
void shift_segment(long ipoint, long *&segments, long &segment_end,
                   double *&output, long increment) {
...
void fill_polynomials_wrapper(double *work, double *delta, long lmax,
                              long mtype) {
  if (mtype == 1) {
    work[0] = delta[0];
    work[1] = delta[1];
...
void dot_multi_moments_cube(long nvector, double *data, UniformGrid *ugrid,
  ...
  if (nvec != 0) {
    throw std::domain_error(
        "dot_multi_moments_cube only works for non-periodic grids.");
  }
  if (lmax < 0) {
