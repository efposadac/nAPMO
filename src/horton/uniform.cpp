...
UniformGrid::UniformGrid(double *_origin, double *_grid_rvecs, long *_shape,
                         long *_pbc) {
  for (int i = 0; i < 3; i++) {
    origin[i] = _origin[i];
    shape[i] = _shape[i];
...
Cell *UniformGrid::get_grid_cell() { return new Cell(grid_rvecs, 3); }

void UniformGrid::set_ranges_rcut(double *center, double rcut,
  ...
  printf("ranges [%li,%li,%li] [%li,%li,%li]\n", ranges_begin[0],
         ranges_begin[1], ranges_begin[2], ranges_end[0], ranges_end[1],
         ranges_end[2]);
#endif
}
...
double UniformGrid::dist_grid_point(double *center, long *i) {
  double delta[3];
  delta[0] = origin[0] - center[0] + i[0] * grid_rvecs[0] +
             i[1] * grid_rvecs[3] + i[2] * grid_rvecs[6];
  delta[1] = origin[1] - center[1] + i[0] * grid_rvecs[1] +
             i[1] * grid_rvecs[4] + i[2] * grid_rvecs[7];
  delta[2] = origin[2] - center[2] + i[0] * grid_rvecs[2] +
             i[1] * grid_rvecs[5] + i[2] * grid_rvecs[8];
...
void UniformGrid::delta_grid_point(double *center, long *i) {
  ...
  center[0] = origin[0] - center[0] + i[0] * grid_rvecs[0] +
              i[1] * grid_rvecs[3] + i[2] * grid_rvecs[6];
  center[1] = origin[1] - center[1] + i[0] * grid_rvecs[1] +
              i[1] * grid_rvecs[4] + i[2] * grid_rvecs[7];
  center[2] = origin[2] - center[2] + i[0] * grid_rvecs[2] +
              i[1] * grid_rvecs[5] + i[2] * grid_rvecs[8];
}
...
UniformGridWindow::UniformGridWindow(UniformGrid *ugrid, long *_begin,
                                     long *_end)
    : ugrid(ugrid) {
...
void UniformGridWindow::extend(double *cell, double *local) {
  ...
  for (long ipoint = r3i.get_npoint() - 1; ipoint >= 0; ipoint--) {
    ...
    printf("ipoint=%li  j={%li, %li, %li}  jwrap={%li, %li, %li}\n", ipoint,
           j[0], j[1], j[2], jwrap[0], jwrap[1], jwrap[2]);
#endif
...
double *UniformGridWindow::get_pointer(double *array, long *i) {
  return array +
         ((i[0] - begin[0]) * (end[1] - begin[1]) + (i[1] - begin[1])) *
             (end[2] - begin[2]) +
         (i[2] - begin[2]);
}
...
Range3Iterator::Range3Iterator(const long *ranges_begin, const long *ranges_end,
                               const long *shape)
...
Block3Iterator::Block3Iterator(const long *begin, const long *end,
                               const long *shape)
...
void Block3Iterator::set_cube_ranges(long *b, long *cube_begin,
                                     long *cube_end) {
  for (int i = 0; i < 3; i++) {
    long offset = b[i] * shape[i];
    cube_begin[i] = offset;
    if (cube_begin[i] < begin[i])
      cube_begin[i] = begin[i];
    cube_begin[i] -= offset;
