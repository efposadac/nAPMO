...
void system_init_cuda(System *sys, System *sys_d)
{
  ...
  cudaMemcpy(sys_d->particle_number, sys->particle_number, bytes_int, cudaMemcpyHostToDevice);
  cudaMemcpy(sys_d->particle_radii, sys->particle_radii, bytes_double, cudaMemcpyHostToDevice);
  cudaMemcpy(sys_d->particle_origin, sys->particle_origin, 3 * bytes_double, cudaMemcpyHostToDevice);
